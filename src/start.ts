...
const securityMiddleware = createMiddleware().server(async ({ next }) => {
  try {
    ...
    const secHeaders: [string, string][] = [
      ...
      ["Content-Security-Policy", `default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; img-src 'self' data: https:; frame-src https://www.google.com; ${import.meta.env.DEV ? "connect-src 'self' http://localhost http://127.0.0.1 ws://localhost:* ws://127.0.0.1:* http://localhost:* http://127.0.0.1:*;" : "connect-src 'self';"}`],
    ];
