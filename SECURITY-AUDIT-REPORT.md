...
| **Skor Keamanan** | **3 / 10 (KRITIS - Tidak Siap Produksi)** |
...
**Hasil audit menemukan 13 temuan keamanan:**

| Severity | Jumlah | Status |
|----------|--------|--------|
| KRITIS (P0) | 3 | Harus diperbaiki sebelum deploy |
| TINGGI (P1) | 4 | Harus diperbaiki dalam sprint berikutnya |
| SEDANG (P2) | 4 | Dijadwalkan untuk perbaikan |
| RENDAH (P3) | 2 | Perbaikan opsional |

**Kesimpulan utama:** Aplikasi memiliki celah kritis pada sistem autentikasi yang sepenuhnya berjalan di client-side. Siapapun dapat bypass login admin melalui browser console. Aplikasi **tidak boleh di-deploy ke production** sebelum temuan P0 diperbaiki.
...
- [ ] **SEC-001** — Hapus kredensial hardcoded dari `admin.tsx:887`
- [ ] **SEC-001** — Implementasi login API di backend dengan password hashing (bcrypt)
- [ ] **SEC-002** — Ganti sessionStorage dengan server-side session (HTTP-only cookie)
- [ ] **SEC-002** — Hapus logika `seal_admin_auth` dari frontend
- [ ] **SEC-003** — Tambahkan auth middleware di semua endpoint PHP
- [ ] **SEC-003** — Kirim auth token di setiap API request dari frontend

### P1 — Tinggi (Sprint Berikutnya)

- [ ] **SEC-004** — Implementasi CSRF token generation di server
- [ ] **SEC-004** — Validasi CSRF token di setiap POST request
- [ ] **SEC-005** — Install DOMPurify dan sanitize semua HTML output
- [ ] **SEC-005** — Escape `data.nama_peserta` dan field lain di template sertifikat
- [ ] **SEC-006** — Ganti semua URL hardcoded di `index.tsx` dengan `API_BASE_URL`
- [ ] **SEC-006** — Konfigurasi `VITE_API_URL` dengan HTTPS URL
- [ ] **SEC-007** — Tambahkan security headers middleware di `start.ts`

### P2 — Sedang (Dijadwalkan)

- [ ] **SEC-008** — Tambahkan validasi MIME type pada file upload
- [ ] **SEC-008** — Terapkan batas ukuran file (max 5MB)
- [ ] **SEC-009** — Implementasi rate limiting di backend PHP
- [ ] **SEC-009** — Tambahkan CAPTCHA pada form publik
- [ ] **SEC-010** — Implementasi Zod validation pada semua form
- [ ] **SEC-011** — Ubah path API dari `/Project-seal-ssh/` ke path generik

### P3 — Rendah (Opsional)

- [ ] **SEC-012** — Tambahkan SRI hash pada CDN scripts atau bundle via npm
- [ ] **SEC-013** — Kondisikan console.error hanya untuk development mode
